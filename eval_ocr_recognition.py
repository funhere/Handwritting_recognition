...
tf.app.flags.DEFINE_string(
    'model_name', 'crnn', 'The name of the architecture to evaluate.')
...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    if FLAGS.model_name == "crnn":
            ...
            network_fn = crnn_net(phase='Test', 
    ...
    if FLAGS.dataset_name == "ocr":
        image,label = tf_utils.read_features(ops.join(FLAGS.dataset_dir, 'ocr_test_000.tfrecord'),num_epochs=None)
    ...
    with tf.variable_scope('crnn'):
        logits, _ = network_fn.build_CRNNnet(images)
    ...
    if  FLAGS.model_name == "crnn":
        network_fn.eval_crnn(FLAGS,decoded,images,labels)
        ...
        predictions = tf.argmax(decoded, 1)
